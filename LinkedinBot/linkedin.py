...
class LinkedinPost:
    ...
    def get_post_data(self, document_pages_limit: int = 15):
        ...
        # post text
        self.text = post_section.find(
            "p", attrs={"class": "attributed-text-segment-list__content"}
        ).text

        # get post details
        post_detail = post_section.find(
            "div", attrs={"class": "main-feed-activity-card__social-actions"}
        )
        if post_detail:
            # get count of reactions and comments
            _reactions = post_detail.find(
                attrs={"data-id": "social-actions__reactions"}
            )
        ...
        if _image_list:
            self.images = [
                item["data-delayed-url"] for item in _image_list.find_all("img")
            ]
        ...
        if _document:
            ...
            self.document.images_list = [
                item["config"]["src"]
                for item in _document["doc"]["coverPages"][:document_pages_limit]
            ]
